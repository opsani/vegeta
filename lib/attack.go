...
func Client(c *fasthttp.Client) func(*Attacker) {
...
func (a *Attacker) hit(tr Targeter, name string) *Result {
	...
	if request.Header.ContentLength() != -1 {
		res.BytesOut = uint64(request.Header.ContentLength())
	}

	statusCode := response.StatusCode()
	if res.Code = uint16(statusCode); statusCode < 200 || statusCode >= 400 {
